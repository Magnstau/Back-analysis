...
# --- Inputs ---
MODEL_FILE = r"C:\Users\Magnu\OneDrive - NTNU\Documents\NTNU-Emner\Master\RS2\Elastic_Model\U_stoll_Elastic.fez"
BASE_OUTPUT_DIR = r"C:\Users\Magnu\OneDrive - NTNU\Documents\NTNU-Emner\Master\RS2\Elastic_Model\RS2_Results_Elastic"
...
Name_of_material = "GlimSkif"

# Target displacement and tolerance
target_change_mm = -10.0  # Target change in distance (mm)
...
min_change_mm = target_change_mm - tolerance_mm  # e.g., -11 mm
max_change_mm = target_change_mm + tolerance_mm  # e.g., -9 mm
...
def run_model_and_get_change(GSI, E_i, D, modeler):
    ...
    GlimSkif = model.getAllMaterialProperties()[0]
    GlimSkif.setMaterialName(Name_of_material)
    GlimSkif.InitialConditions.setUnitWeight(UnitWeight)
    GlimSkif.InitialConditions.setPorosityValue(Porosity)
    GlimSkif.Stiffness.Isotropic.setYoungsModulus(E_m)
    GlimSkif.Stiffness.Isotropic.setPoissonsRatio(Poisson)
    GlimSkif.Strength.setFailureCriterion(StrengthCriteriaTypes.GENERALIZED_HOEK_BROWN)
    GlimSkif.Strength.GeneralizedHoekBrown.setMaterialType(MaterialType.ELASTIC)
    GlimSkif.Strength.GeneralizedHoekBrown.setCompressiveStrength(UCS)
    GlimSkif.Strength.GeneralizedHoekBrown.setMbParameter(mb)
    GlimSkif.Strength.GeneralizedHoekBrown.setSParameter(s)
    GlimSkif.Strength.GeneralizedHoekBrown.setAParameter(a)
